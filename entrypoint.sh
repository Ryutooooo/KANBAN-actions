...
add_section "this time assignees"
echo $ASSIGNEES
...
add_section "got target projects"
...
do
  ...
  do
    if echo $MEMBERS | grep $assignee; then 
      add_section "$assignee was included in the target team"
      ...
      case "$GITHUB_EVENT_NAME" in
        issues)
          ...
          curl -s -X POST -u "$ACTOR:$TOKEN" --retry 3 \
           -H 'Accept: application/vnd.github.inertia-preview+json' \
